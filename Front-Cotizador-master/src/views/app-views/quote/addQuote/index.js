  ...
  const calculatePrices = (product, isMark) => {
    ...
    product.markings.forEach((mark, j) => {
      ...
      product.markings[j].unitPrice = parseInt(
        mark.netPrice /
          (product.markings[j].profit > 0
            ? (100 - product.markings[j].profit) / 100
            : 1) +
          product.markings[j].markingPrice
      );

      

      product.markings[j].totalPrice =
          parseInt(product.markings[j].unitPrice * mark.amount) +
          parseInt(product.markings[j].markingPrice) +
          parseInt(product.markings[j].freight) +
          parseInt(product.markings[j].totalProfit * mark.amount);
      */
    });
                            ...
                            >
                              ...
                              >
                                <Input
                                  prefix="$"
                                  name="totalProfit"
                                  value={Number.parseInt(m.totalProfit)
                                    .toString()
                                    .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
